...
import { createSpyObj, toHyphenCase } from "./helpers";
import { faCoffee, faCircle, farCoffee } from "./icons";
...
describe("the font awesome icon custom element", () => {
  ...
  it("calls the visitor with self during initialization", async done => {
    ...
    expect(visitorSpy.visit).toHaveBeenCalledWith(
      jasmine.any(FontAwesomeIconCustomElement)
    );
    done();
  });
  ...
  [{ prefix: "fas", iconName: "coffee" }, ["fas", "coffee"], "coffee"].forEach(
    icon => {
  ...
  describe("logging an error", () => {
    ...
    [{ prefix: "fas", iconName: "foo" }, ["fas", "foo"], "foo"].forEach(
      icon => {
        it("logs an error with missing icon", async done => {
          /* Arrange */
          component
            .inView(
              `<font-awesome-icon icon.bind="icon">
                         </font-awesome-icon>`
            )
            .boundTo({ icon });
          ...
          expect(logger.error.calls.argsFor(0)[1]).toEqual({
            prefix: "fas",
            iconName: "foo"
          });
          done();
  ...
  it("uses a border", async done => {
    /* Arrange */
    component.inView(
      '<font-awesome-icon icon="coffee" border.bind="true"></font-awesome-icon>'
    );
  ...
  it("uses a fixed width", async done => {
    /* Arrange */
    component.inView(
      '<font-awesome-icon icon="coffee" fixed-width.bind="true"></font-awesome-icon>'
    );
  ...
  it("uses the inverse", async done => {
    /* Arrange */
    component.inView(
      '<font-awesome-icon icon="coffee" inverse.bind="true"></font-awesome-icon>'
    );
  ...
  describe("extra element data", () => {
    it("adds all classes", async done => {
      /* Arrange */
      component.inView(
        `<font-awesome-icon icon="coffee" class-name="foo bar"></font-awesome-icon>`
      );
    ...
    it("adds extra attributes", async done => {
      /* Arrange */
      component.inView(
        `<font-awesome-icon icon="coffee" rel="local"></font-awesome-icon>`
      );
  ...
  describe("using styles", () => {
    it("adds extra styles", async done => {
      ...
      component
        .inView(
          `<font-awesome-icon icon="coffee" style.bind="style">
         </font-awesome-icon>`
        )
        .boundTo({ style });
  ...
  describe("using flip", () => {
    [
      ...
      it("adds horizontal OR vertical flip", async done => {
        ...
        component
          .inView(
            `<font-awesome-icon icon="coffee" flip.bind="flip"></font-awesome-icon>`
          )
    ...
    it("adds horizontal AND vertical flip", async done => {
      /* Arrange */
      component.inView(
        `<font-awesome-icon icon="coffee" flip="both"></font-awesome-icon>`
      );
  ...
  it("adds list item", async done => {
    /* Arrange */
    component.inView(
      `<font-awesome-icon icon="coffee" list-item.bind="true"></font-awesome-icon>`
    );
  ...
  ["right", "left"].forEach(pull => {
    it("uses pull", async done => {
      ...
      component
        .inView(
          `<font-awesome-icon icon="coffee" pull.bind="pull"></font-awesome-icon>`
        )
        .boundTo({ pull });
  ...
  it("uses pulse", async done => {
    /* Arrange */
    component.inView(
      `<font-awesome-icon icon="coffee" pulse.bind="true"></font-awesome-icon>`
    );
  ...
  ["90", "180", "270"].forEach(rotation => {
    it("uses rotation", async done => {
      ...
      component
        .inView(
          `<font-awesome-icon icon="coffee" rotation.bind="rotation"></font-awesome-icon>`
        )
        .boundTo({ rotation });
  ...
  it("uses spin", async done => {
    /* Arrange */
    component.inView(
      `<font-awesome-icon icon="coffee" spin.bind="true"></font-awesome-icon>`
    );
  ...
  [
    ...
    it("uses size", async done => {
      ...
      component
        .inView(
          `<font-awesome-icon icon="coffee" size.bind="size"></font-awesome-icon>`
        )
        .boundTo({ size });
  ...
  describe("using transform", () => {
    ...
    it("uses an object", async done => {
      ...
      component
        .inView(
          `<font-awesome-icon icon="coffee" transform.bind="transform">
        </font-awesome-icon>`
        )
        .boundTo({ transform });
  ...
  describe("using mask", () => {
    const maskArgs = [{ prefix: "fas", iconName: "circle" }, ["fas", "circle"]];
    ...
    maskArgs.forEach(mask => {
      it("works with a string or array", async done => {
        ...
        component
          .inView(
            '<font-awesome-icon icon="coffee" mask.bind="mask"></font-awesome-icon>'
          )
          .boundTo({ mask });
    ...
    maskArgs.forEach(mask => {
      it("changes on property update", async done => {
        ...
        component
          .inView(
            `<font-awesome-icon icon="coffee" mask.bind="mask">
                         </font-awesome-icon>`
          )
          .boundTo(context);
        ...
        const $clipPathAfter = (await component.waitForElement(
          "clipPath"
        )) as Element;
  ...
  ["1x", "2x"].forEach(stack => {
    it("stacks", async done => {
      ...
      component
        .inView(
          `<font-awesome-icon icon="coffee" stack.bind="stack"></font-awesome-icon>`
        )
        .boundTo({ stack });
  ...
  describe("symbol", () => {
    it("will not create a symbol", async done => {
      ...
      component
        .inView('<font-awesome-icon icon.bind="faCoffee"></font-awesome-icon>')
        .boundTo({ faCoffee });
    ...
    it("creates a symbol", async done => {
      ...
      component
        .inView(
          `<font-awesome-icon icon.bind="faCoffee" symbol="coffee-icon">
                       </font-awesome-icon>`
        )
        .boundTo({ faCoffee });
    ...
    it("creates a symbol when the property changes", async done => {
      ...
      component
        .inView(
          `<font-awesome-icon icon="coffee" symbol.bind="symbol">
                       </font-awesome-icon>`
        )
        .boundTo(context);
  ...
  describe("title property", () => {
    it("adds a title element", async done => {
      /* Arrange */
      component.inView(
        '<font-awesome-icon icon="coffee" title="foobar"></font-awesome-icon>'
      );
    ...
    it("changes the title name when the prop changes", async done => {
      ...
      component
        .inView(
          `<font-awesome-icon icon="coffee" title.bind="title">
                       </font-awesome-icon>`
        )
        .boundTo(context);
  ...
  [
    ...
    it("recompiles when any bindable property changes", async done => {
      ...
      const $afterPropChange = await component.waitForElement(
        `.${rec.className}`
      );
  ...
  describe("the icon prefix", () => {
    it("accepts a prefix", async done => {
      /* Arrange */
      component.inView(
        `<font-awesome-icon icon="coffee" prefix="far"></font-awesome-icon>`
      );
    ...
    it("recompiles when the prefix changes", async done => {
      ...
      component
        .inView(
          `<font-awesome-icon icon="coffee" prefix.bind="prefix"></font-awesome-icon>`
        )
        .boundTo(context);
