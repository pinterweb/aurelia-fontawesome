...
describe('aurelia setup', () => {
  ...
  it('registers the custom element with its requirements', () => {
    ...
    expect(container.registerSingleton.calls.argsFor(0)[0]).toBe(PluginIconVisitor);
    expect(container.registerSingleton.calls.argsFor(0)[1]).toBe(PluginIconVisitor);
  });
  ...
  it('register icons and icon options when both exists', () => {
    ...
    let registerInstance:IconConfigurationVisitor = null;
    ...
    expect(container.registerInstance.calls.argsFor(0)[0]).toBe(IconVisitorInjectionKey);
    expect(container.registerInstance.calls.argsFor(0)[1]).toEqual(jasmine.any(IconConfigurationVisitor));
  });
});
