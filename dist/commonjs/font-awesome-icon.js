...
var FontAwesomeIconCustomElement = (function () {
    ...
    FontAwesomeIconCustomElement.prototype.renderIcon = function () {
        ...
        var classes = (_a = {
            ...
            _a);
        var classObj = utils_1.objectWithKey('classes', __spreadArrays(Object.keys(classes).filter(function (key) { return classes[key]; }), this.className.split(' ')));
        var otherIconParams = __assign(__assign({}, utils_1.objectWithKey('mask', normalizeIconArgs(this.mask))), utils_1.objectWithKey('transform', typeof this.transform === 'string'
            ? fontawesome_svg_core_1.parse.transform(this.transform)
            : this.transform));
        var renderedIcon = fontawesome_svg_core_1.icon(this.iconLookup, __assign(__assign(__assign({}, classObj), otherIconParams), { attributes: this.getOtherAttributes(), styles: this.style, symbol: this.symbol, title: this.title }));
        if (!renderedIcon) {
